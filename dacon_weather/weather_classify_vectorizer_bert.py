...
sample_submission = pd.read_csv('../_data/open/sample_submission.csv')
...
train = train[['과제명', '내역사업명', '요약문_한글키워드', '요약문_연구내용', 'label']]
test = test[['과제명', '내역사업명', '요약문_한글키워드', '요약문_연구내용']]
train['요약문_연구내용'].fillna('NAN', inplace=True)
test['요약문_연구내용'].fillna('NAN', inplace=True)

train['data'] = train['과제명']+train['요약문_연구내용']+ train['내역사업명']+ train['요약문_한글키워드']
test['data'] = test['과제명']+test['요약문_연구내용']+ test['내역사업명']+ test['요약문_한글키워드']
...
def clean_text(sent):
    sent_clean = re.sub('[a-zA-z]','',str(sent))
    sent_clean = re.sub('[-=+,#○▷▶■ ●□/\?:^$.@*\"*~&%`!\\`|\(\)\[\]\<\>`\'...>]', '', str(sent)) 
    sent_clean = re.sub("[^가-힣ㄱ-ㅎㅏ-ㅣ\\s]", " ", str(sent))
    sent_clean = [word for word in sent if not word in stopwords]  # 불용어 처리 
    return sent_clean
...
sample_submission.to_csv('../_data/bert_baseline.csv', index=False)