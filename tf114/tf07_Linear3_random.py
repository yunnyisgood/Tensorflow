...
w = tf.Variable(random_normal([1]), dtype=tf.float32) # random_normal -> 정규분포에 의한 random 
b = tf.Variable(random_normal([1]), dtype=tf.float32) # random이지만, random_seed를 바꿔주지 않으면 계속 동일한 값으로 실행됨 
