...
# modeling
w1 = tf.compat.v1.get_variable('w1', shape=[3, 3, 1, 32]) #  자동으로 초기값을 넣어준다, 단 name, shape를 꼭 넣어줘야!
...
# Layer 2
w2 = tf.compat.v1.get_variable('w2', shape=[3, 3, 32, 64]) # 이 때 32는 그 전 layer에서 output의 수가 되어야
...
# Layer 3
w3 = tf.compat.v1.get_variable('w3', shape=[3, 3, 64, 128]) # 이 때 32는 그 전 layer에서 output의 수가 되어야
...
w4 = tf.compat.v1.get_variable('w4', shape=[2, 2, 128, 64], 
                    ) # 이 때 32는 그 전 layer에서 output의 수가 되어야
...
# L5
w5 = tf.compat.v1.get_variable('w5', shape=[256, 64],
# w5 = tf.compat.v1.get_variable('w5', shape=[256, 64],
                        )
b5 = tf.Variable(tf.random.normal([64]))
...
b6 = tf.Variable(tf.random.normal([32]))
