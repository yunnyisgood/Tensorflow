...
# '''                단어사전의 개수(=라벨의 개수)    문장의 길이(=행의 최대의 길이)           '''
# '''  output_dim은 dnn의 units, cnn의 filter 동일   '''
# ''' -> RNN, CNN과는 다르게 인풋은 의미 없음. 인풋은 2차원 벡터화하기 위해 필요한 것 뿐
#     -> 단어 사전의 개수가 중요하다  '''
# # model.add(Embedding(27, 77))  
# # input_length를 안 주었을 때 고정으로 주게 된다
# model.add(LSTM(32))
# model.add(Dense(1, activation='sigmoid'))


Input = Input(shape=(5, )) # (13, 5)이기 때문에 
# Input = Input(shape=(None, ))  
xx = Embedding(input_dim=28, output_dim=11)(Input)
xx = LSTM(32)(xx)
