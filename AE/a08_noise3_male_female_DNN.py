...
def autocoder(hidden_layer_size):
    model = Sequential()
    model.add(Dense(units=hidden_layer_size, input_shape=(80*80*3, ),
                        activation='relu')) # dense에서 output은 units
    model.add(Dense(units=80*80*3, activation='sigmoid'))
    return model
...
for i, ax in enumerate([ax11, ax12, ax13]):
    ax.imshow(x_pred_noised[random_img[i]].reshape(80, 80, 3), cmap='gray')
    if i == 0:
