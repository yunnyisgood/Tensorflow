...
'''# modeling
model = Sequential()
model.add(LSTM(units=32, activation='relu', input_shape=(28, 28), return_sequences=True))
...
hist = model.fit(x_train, y_train, epochs=100, verbose=1, callbacks=[es, cp], validation_split=0.2,
shuffle=True, batch_size=256)
end_time = time.time() - start_time

model.save('./_save/ModelCheckPoint/keras47_MCP_fashion.h5')'''
...
Conv1D 
걸린시간:  403.70621728897095
loss:  0.3804386854171753
accuracy:  0.8708999752998352

MCP [전] -> early stopping 지점
걸린시간:  310.71823143959045
loss:  0.48969852924346924
accuracy:  0.8230000138282776

MCP [후] -> modelCheckPoint 지점 
loss:  0.43476077914237976
