...
'''
뉴스토픽 경진대회 더 시도해볼만한 부분
1. 형태소 분석을 통한 어큐러시 상승
2. 
...
dataset['title'] = dataset['title'].dropna(axis=0).apply(lambda x : clean_text(x))
dataset['topic_idx'] = dataset['topic_idx'].dropna(axis=0)
x_pred['title'] = x_pred['title'].dropna(axis=0).apply(lambda x : clean_text(x))

#형태소 분석
okt = Okt()
dataset['title_analysis'] = np.nan
x_pred['title_analysis'] = np.nan

x_temp_list = []
for sentence in dataset['title']:
    temp = []
    temp = okt.normalize(sentence)
    temp  = okt.morphs(sentence) #  문장에서 명사 추출 
    temp = [word for word in temp if not word in stopwords]  # 불용어 처리 
    x_temp_list.append(temp)
dataset['title_analysis'] = x_temp_list

x_temp_list2 = []
for sentence in x_pred['title'] :
    temp = []
    temp = okt.normalize(sentence)
    temp  = okt.morphs(sentence) #  문장에서 명사 추출 
    temp = [word for word in temp if not word in stopwords]  # 불용어 처리 
    x_temp_list2.append(temp)
x_pred['title_analysis'] = x_temp_list2

x = dataset['title_analysis']
x_pred = x_pred['title_analysis']
print(x[:10])
print(type(x))

x = list(itertools.chain(*x))
x_pred = list(itertools.chain(*x_pred))
y = dataset['topic_idx'].to_numpy()
print(x[:10])
...
vectorizer = TfidfVectorizer(analyzer='word', sublinear_tf=True,
 ngram_range=(1, 2), max_features=40000, binary=False)
...
# x_train_list = []
# for i in x_train:
#     x_train_list.append(vectorizer.fit_transform(i))
# x_train = x_train_list.toarray()

# x_test_list = []
# for i in x_test:
#     x_test_list.append(vectorizer.transform(i))
# x_test = x_test_list.toarray()

# x_pred_list = []
# for i in x_pred:
#     x_pred_list.append(vectorizer.transform(i))
# x_pred = x_pred_list.toarray()
...
# model = Sequential()
# model.add(Dense(100, input_dim = 55000, activation = "relu"))
# # model.add(Dropout(0.8))
# model.add(Bidirectional(Dense(64, activation = "relu")))
# # model.add(Dropout(0.2))
# model.add(Bidirectional(Dense(7, activation = "softmax")))

model = Sequential()
model.add(Dense(100, input_dim = 40000, activation = "relu"))
model.add(Dropout(0.8))
...
#             epochs=100,
#             batch_size=10)
hist = model.fit(x_train, y_train, epochs=100, batch_size=512, verbose=1, callbacks=[es, reduce_lr], validation_split=0.25)
